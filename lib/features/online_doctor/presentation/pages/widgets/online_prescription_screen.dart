...
class OnlinePrescriptionScreen extends StatefulWidget {
  final OnlineDoctorBookingItem booking;

  const OnlinePrescriptionScreen({Key? key, required this.booking}) : super(key: key);

  @override
  State<OnlinePrescriptionScreen> createState() => _OnlinePrescriptionScreenState();
}

class _OnlinePrescriptionScreenState extends State<OnlinePrescriptionScreen> {
  ...
  Future<Uint8List> _getLogoBytes() async {
    if (_logoCaptured && _logoBytes != null) return _logoBytes!;
    if (!_captureCompleter.isCompleted) await _captureCompleter.future;
    if (_logoBytes == null) return await _createFallbackLogo();
    return _logoBytes!;
  }
  ...
  Future<void> _downloadPDF() async {
    ...
    try {
      ...
      final pdf = await _generatePDF(logoBytes);
      final output = await getTemporaryDirectory();
      final timestamp = DateTime.now().millisecondsSinceEpoch;
      final file = File('${output.path}/prescription_${widget.booking.bookingId}_$timestamp.pdf');
            ...
            children: [
                  ...
                  pw.Expanded(
                    child: pw.Column(
                      ...
                      children: [
                        ...
                        if (booking.specialization.isNotEmpty)
                          pw.Text(
              ...
              if (booking.medicines.isNotEmpty) ...[
                ...
                ...booking.medicines.map((med) => pw.Row(
                  children: [
                    ...
                    pw.Text('${med.medicine}  (${med.medicineTime})'),
                  ],
                )).toList(),
                pw.SizedBox(height: 8),
              ],
              ...
              if (booking.tests.isNotEmpty) ...[
                ...
                ...booking.tests.map((test) => pw.Row(
                  children: [
                    pw.Text('• ', style: pw.TextStyle(fontSize: 14)),
                    pw.Text('${test.test}  (${test.testInstruction})'),
                  ],
                )).toList(),
                pw.SizedBox(height: 8),
              ],
