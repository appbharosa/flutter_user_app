...
import '../models/lab_coupon_model.dart';

abstract class LabCouponRemoteDataSource {
  Future<List<LabCouponModel>> getCoupons();
  Future<Map<String, dynamic>> applyCoupon(String couponCode, double amount);
}

class LabCouponRemoteDataSourceImpl implements LabCouponRemoteDataSource {
  final DioClient dioClient;
  LabCouponRemoteDataSourceImpl(this.dioClient);

  @override
  Future<List<LabCouponModel>> getCoupons() async {
    try {
      final response = await dioClient.dio.get(AppUrls.labCoupons);
      if (response.data['status'] == 200) {
        final resultList = response.data['result'] as List;
        if (resultList.isEmpty) return [];
        final couponsList = resultList[0] as List;
        return couponsList.map((json) => LabCouponModel.fromJson(json)).toList();
      } else {
